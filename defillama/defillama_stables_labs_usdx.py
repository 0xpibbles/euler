...
# 1. Query the latest date for Stables Labs USDX from Dune
...
protocol = "Stables Labs USDX"
latest_date = get_latest_date_from_dune(protocol)

# 2. Fetch API data
url = "https://api.llama.fi/protocol/stables-labs-usdx"
...
for entry_date, entry in latest_entries.items():
    if not latest_date or entry_date > latest_date:
        ...
        new_rows.append([entry_date.isoformat(), protocol, chain, total_liquidity])

# 5. If there is new data, append to CSV and upload
csv_file_path = 'defillama_stables_labs_usdx_new.csv'
if new_rows:
    with open(csv_file_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['date', 'protocol', 'chain', 'total_liquidity_usd'])
    ...
    try:
        with open(csv_file_path, "rb") as data:
            response = dune.insert_table(
                namespace="0xpibs",
                table_name="defillama_stables_labs_usdx_arbitrum",
                data=data,
